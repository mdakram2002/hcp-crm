...
def get_or_create_draft(db: Session, session_id: str, user_id: Optional[int] = None) -> Interaction:
    ...
    if draft is None:
        draft = Interaction(session_id=session_id, status="draft", attendees=[], materials_shared=[], samples_distributed=[], follow_up_actions=[], ai_suggested_follow_ups=[])
...
def apply_updates(db: Session, draft: Interaction, updates: dict) -> dict:
    ...
    if "topics_discussed" in updates or "outcomes" in updates:
        text_to_embed = " ".join(part for part in [getattr(draft, "topics_discussed", None), getattr(draft, "outcomes", None)] if part).strip()
        if text_to_embed:
...
def get_dashboard_summary(db: Session) -> dict:
    ...
    material_items = [{"name": name, "count": count, "item_type": "material"} for name, count in materials_counter.most_common(5)]
    sample_items = [{"name": name, "count": count, "item_type": "sample"} for name, count in samples_counter.most_common(5)]
    ...
    needs_attention = []
    for rep in reps:
        recent = [i for i in interactions if i.user_id == rep.id and _parse_date(i.date) and _parse_date(i.date) >= week_start]
        if not recent:
            needs_attention.append({"email": rep.email, "role": rep.role, "last_interaction_date": None})

    return {
        "total_interactions_this_week": len(week_interactions),
        "total_interactions_this_month": len(month_interactions),
        "sentiment_breakdown": {"Positive": sentiment_breakdown.get("Positive", 0), "Neutral": sentiment_breakdown.get("Neutral", 0), "Negative": sentiment_breakdown.get("Negative", 0)},
